...
- If the forwarded request includes `--background`, pass it to `task` so the companion creates an owned detached worker. Strip `--wait` before calling `task`. Neither flag belongs in the natural-language task text.
