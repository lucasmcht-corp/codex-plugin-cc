...
- `--background` selects an owned detached worker and must be forwarded to `task`. `--wait` is not forwarded. Neither flag belongs in the natural-language task text.
